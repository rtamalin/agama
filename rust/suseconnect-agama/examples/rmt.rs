...
pub fn main() {
    ...
    if args.len() < 2 {
        eprintln!("Usage: rmt <url> [<regcode> [<cert_path>]]");
        exit(1);
    }
    ...
    let cert_path = args.get(3).cloned();
    ...
    // call reload_certificates() - technically a no-op but ensures libsuseconnect.so
    // loaded and initialiased
    if let Err(e) = reload_certificates() {
        eprintln!("Warning: reload_certificates() failed: {}. Continuing anyway.", e);
    } else {
        println!("Successfully reloaded certificates.");
    }
